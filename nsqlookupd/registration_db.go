...
// 如果有 channel 则 Category 为 "channel"，key 为 topic，SubKey 为 channel
// 注册 topic 时，category 为 "topic"，key 为 topic
...
// 这种传输结构体在各自文件定义，通过 struct 区分是
// 不可取的，如果一方改变怎么办？
// 当然这个版本比较早，还是用 grpc 好一点。
...
func (r *RegistrationDB) FindProducers(category string, key string, subkey string) Producers {
	...
	for k, producers := range r.registrationMap {
		...
		for _, producer := range producers {
			found := false
			// add 的时候没有检验，所以有可能会多次注册 producer
...
// 通过 Registrations 转为所有的 key []string
...
// 通过 Registrations 转为所有的 subKeys []string
